...
tweetids = [int(tweetid.strip()) for tweetid in open(data_location) if not tweetid == '\n']
...
for i in range(c, iterations):
    try:
        ...
        with open('all_metadata.json', 'a') as all_metadata_file:
            json.dump(statuses, all_metadata_file, cls=JSONEncoder)

        output = open("tweets_metadata.json", "a")
        ...
        for status in statuses:
            ...
            if "coordinates" in json_load:
                if json_load["coordinates"]:

                    print(json_load['coordinates']['coordinates'])
                    if json_load['coordinates']['type'] == 'Point':
                        coordinates = json_load['coordinates']['coordinates']
                    else:
                        points = json_load["coordinates"]["coordinates"]
                        area = Polygon(points)
                        coordinates = list(area.centroid)


            if coordinates:
                output.write('{},{},{}\n'.format(tweetid, *coordinates))
        ...
        output.close()
            ...
            #         cur2 = conn.cursor()
            #         cur2.execute(
            #             "INSERT INTO " + tweets_table + "(tweetid, userid, created_at, text, lang, in_reply_to_status_id, in_reply_to_user_id, in_reply_to_screen_name, retweet_count, favorite_count, quote_count, reply_count, source, possibly_sensitive, latitude, longitude, place, hashtags, urls, user_mentions, media, is_quote_status, quoted_status_id, quoted_status, retweeted_status) VALUES(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
            #             (tweetid, userid, tweet_created_at, text, lang, in_reply_to_status_id, in_reply_to_user_id,
            #              in_reply_to_screen_name, retweet_count, favorite_count, quote_count, reply_count, source,
            #              possibly_sensitive, latitude, longitude, json.dumps(place), json.dumps(hashtags),
            #              json.dumps(urls), json.dumps(user_mentions), json.dumps(media), is_quote_status,
            #              quoted_status_id, json.dumps(quoted_status), json.dumps(retweeted_status),))
            #         cur2.close()
            ...
            #     cur = conn.cursor()
            #     cur.execute(
            #         "INSERT INTO " + users_table + "(userid, name, screen_name, location, description, verified, followers_count, friends_count, listed_count, favorites_count, statuses_count, created_at, time_zone, geo_enabled, utc_offset, lang, profile_image_url, url, protected) VALUES(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
            #         (userid, name, screen_name, location, description, verified, followers_count, friends_count,
            #          listed_count, favorites_count, statuses_count, user_created_at, time_zone, geo_enabled, utc_offset,
            #          lang, profile_image_url, url, protected,))
            #     cur.close()
